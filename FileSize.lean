...
anyway. I suggest:
  ...
  | file : System.FilePath -> IO.FS.Metadata -> Tree
  | dir : System.FilePath -> IO.FS.Metadata -> List Tree -> Tree
...
partial def displayFileSize (path : String) (indent : Nat := 0): IO Unit := do
  ...
  if (<- System.FilePath.isDir path) then
    let children <- System.FilePath.readDir path
    for child in children do
        let path := child.root.toString ++ (String.singleton System.FilePath.pathSeparator) ++ child.fileName
...
def getTree (path : System.FilePath): IO (Tree IO.FS.Metadata) := do
  let entry : IO.FS.DirEntry := {root := path, fileName := ""}
  let metadata <- System.FilePath.metadata path
  let pathIsFile := !(<- System.FilePath.isDir path)
  if pathIsFile then
    return Tree.file entry metadata
  else
    let children <- System.FilePath.readDir path
    let childTrees <- children.mapM (fun child => getTree (System.FilePath.join child.root child.fileName))
