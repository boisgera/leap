...
/-!
TODO: refactor. Extract a lemma that there is a decreasing sequence of
positive numbers that converges to 0 and translate that the elementary
proposition that we need.
-/

lemma e_he : ∃ (a : ℕ → ℝ),
    (∀ n, a n ≠ 0) ∧ (∀ ε > 0, ∃ N, ∀ n ≥ N, |a n| < ε) := by
  admit

/-! TODO: prove then use in the proof below -/
...
example (f : ℝ → ℝ) (x ℓ : ℝ) :
  ...
  have : (∀ ε > 0, ∃ N, ∀ n ≥ N, |a n - x| < ε) := by
    ...
    have (n : ℕ) : n ≥ N → (1 / 2 ^ n : ℝ) < ε' := by
      -- Subproof generated by Claude
      intro hn
      have hlog_lt : Real.logb 2 (1 / ε') < ↑n := by
        have h1 : Real.logb 2 (1 / ε') ≤ ⌈Real.logb 2 (1 / ε')⌉₊ := Nat.le_ceil _
        have h2 : ⌈Real.logb 2 (1 / ε')⌉₊ + 1 ≤ n := hn
        exact h1.trans_lt (by exact_mod_cast Nat.lt_of_succ_le h2)
      have hinvε_lt : 1 / ε' < 2 ^ n := by
        have h := Real.rpow_lt_rpow_of_exponent_lt (by norm_num : (1:ℝ) < 2) hlog_lt
        rwa [Real.rpow_natCast,
            Real.rpow_logb (by norm_num) (by norm_num) (by positivity)] at h
      have h2pos : (0 : ℝ) < 2 ^ n := by positivity
      rw [div_lt_iff₀ h2pos]
      rwa [div_lt_iff₀ ε'_pos, mul_comm] at hinvε_lt
    use N
    intro n n_ge_N
    specialize this n n_ge_N
    simp only [a]
    have k := ha n |>.2.1
    exact lt_trans k this
