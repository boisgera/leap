...
lemma he : ∀ ε > 0, ∃ N, ∀ n ≥ N, |1 / (n + 1 : ℝ)| < ε := by
  intro ε ε_pos
  let N := ⌈1 / ε⌉₊
  use N
  intro n n_ge_N
  simp only [N] at n_ge_N
  have := Nat.le_ceil (1 / ε)
  have := le_trans this n_ge_N
  field_simp at this
  rw [abs_of_nonneg (by admit)]
  --
...
example (f : ℝ → ℝ) (x ℓ : ℝ) :
  ...
  push_neg at h
  have h' (n : ℕ) := h (δ := 1 / (n + 1)) (by positivity)
